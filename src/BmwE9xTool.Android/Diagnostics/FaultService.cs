...
public sealed class FaultService
{
    ...
    public FaultService(EdiabasSession session, AppLog log)
    {
        _session = session;
        _log = log;
    }

    public async Task<IReadOnlyList<FaultRecord>> ScanAsync(
        IProgress<string>? progress = null,
        CancellationToken ct = default)
    {
        var faults = new List<FaultRecord>();

        foreach (var ecu in E90Catalog.Common)
        {
            ...
            try
            {
                ...
                foreach (var set in result.Sets.Skip(1))
                {
                    ...
                    bool? present = ParsePresent(set);
                    if (code == null && text == null && !raw.Keys.Any(k => k.StartsWith("F_", StringComparison.OrdinalIgnoreCase)))
                        continue;
            ...
            {
                _log.Add($"Fault scan {ecu.Name} skipped: {ex.Message}");
            }
        }
    ...
    private static bool? ParsePresent(JobSet set)
    {
        ...
        if (value.Contains("JA", StringComparison.OrdinalIgnoreCase) ||
            value.Contains("PRESENT", StringComparison.OrdinalIgnoreCase)) return true;
        if (value.Contains("NEIN", StringComparison.OrdinalIgnoreCase) ||
            value.Contains("NOT", StringComparison.OrdinalIgnoreCase)) return false;
        return null;
    }
}
