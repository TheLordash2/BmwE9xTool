...
public sealed class MainActivity : Activity
{
    ...
    private void BuildUi()
    {
        ...
        root.AddView(title);
        root.AddView(new TextView(this) { Text = "E8x/E9x diagnostics, fault reading and FA/VO editing" });
        ...
        arm.CheckedChange += (_, e) => { _writeArmed = e.IsChecked; RefreshState(); };
        root.AddView(arm);
        ...
        experimental.CheckedChange += (_, e) => { _experimentalWriteAccepted = e.IsChecked; RefreshState(); };
    ...
    private void RegisterUsbReceiver()
    {
        ...
        {
            RunOnUiThread(() => { _log.Add("USB permission " + (granted ? "granted" : "denied")); RefreshState(); });
        });
...
#pragma warning restore CA1422
    ...
    private async Task ReadVinAsync()
    {
        await BusyAsync("Reading VIN...", async () =>
        {
            ...
            _output.Text = §"VIN read from car: {_vin}\nLocal safety match: {(match ? "YES" : "NO")}";
        });
    }
    ...
    private async Task ReadFaAsync()
    {
        await BusyAsync("Reading FA/VO...", async () =>
        {
            ...
            var cas = _orders.FirstOrDefault(x => x.Source.Equals("CAS", StringComparison.OrdinalIgnoreCase)) ?? _orders[0];
    ...
    private async Task ScanFaultsAsync()
    {
        await BusyAsync("Scanning faults...", async () =>
        {
            ...
            _output.Text = records.Count == 0
                ...
                : string.Join(Environment.NewLine, records.Select(f =>
    ...
    {
        if (_editor == null) { _output.Text = "Read FA/VO first."; return; }
        try
        {
            var optionCode = OptionCatalog.Normalize(_optionInput.Text ?? "");
            var changed = add ? _editor.Add(optionCode) : _editor.Remove(optionCode);
            _optionInput.Text = "";
            HideKeyboard(_optionInput);

            _output.Text =
                §"{(add ? "Add" : "Remove")} ${optionCode}: {(changed ? "STAGED" : "NO CHANGE")}\n\n" +
                §"Original:\n{_editor.Original.StandardFa}\n\nProposed:\n{_editor.BuildStandardFa()}";
            _log.Add(§"{(add ? "Staged add" : "Staged remove")} ${optionCode}");
        }
        catch (Exception ex) { ShowError("Option edit failed", ex); }
        finally { RefreshState(); }
    }
    ...
    {
        if (_editor == null) { _output.Text = "Nothing staged. Read FA/VO and edit an option first."; return; }

        var voltage = _session.BatteryVoltage();
        var gate = new WriteGateState(
            VehicleConfig.VinMatches(_vin), _backupPath != null, voltage.HasValue, voltage,
            _writeArmed, _experimentalWriteAccepted);
        ...
        await BusyAsync("Writing and verifying FA...", async () =>
        {
            ...
            if (targets.Count == 0)
                throw new InvalidOperationException("No CAS/FRM FA write target found in imported ECU data.");

            var written = await _writer.WriteAndVerifyAsync(_editor.Original, _editor.BuildStandardFa());
            _orders = await _faService.ReadAllAvailableAsync();
            _editor = new FaEditor(
                _orders.FirstOrDefault(x => x.Source.Equals("CAS", StringComparison.OrdinalIgnoreCase)) ?? _orders[0]);

            _output.Text = "FA WRITE VERIFIED\nTargets: " + string.Join(", ", written) +
                           "\n\nCurrent FA:\n" + _editor.Original.StandardFa;
            _backupPath = null;
            _writeArmed = false;
        });
    ...
    private void ShowVehicleOrders(IReadOnlyList<VehicleOrder> orders)
    {
        ...
        if (orders.Count > 1)
        {
            ...
            var same = orders.All(x => string.Equals(x.StandardFa, baseline, StringComparison.OrdinalIgnoreCase));
    ...
    {
        try { SetButtons(false); _status.Text = message; await operation(); }
        catch (Exception ex) { ShowError(message, ex); }
        finally { SetButtons(true); RefreshState(); }
    }
